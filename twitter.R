...
tweetgainer.texts <- lapply(tweetgainer, 
                            function(t) {
                              iconv(t$getText(), 
                                    "latin1", "ASCII", sub="")
                            })
...
scores1 <- sapply(tweetgainer.texts, 
                  sentiment, 
                  pos.words, neg.words)


scores2 <- sapply(tweetloser.texts, 
                  sentiment, 
                  pos.words, neg.words)
...
scores1.na <- sapply(tweetgainer.texts, 
                     sentiment.na, 
                     pos.words, neg.words)

table(scores1.na)

barplot(table(scores1.na), 
        xlab="Score", ylab="Count",
...
scores2.na <- sapply(tweetloser.texts, 
                     sentiment.na, 
                     pos.words, neg.words)

table(scores2.na)

barplot(table(scores2.na), 
        xlab="Score", ylab="Count",
        ylim=c(0,40), col="cyan")
...
getSymbols.yahoo(c('MSFT','GOOG','AAPL','GSPC'),env=globalenv(),
                 from = "2015-01-01",to = Sys.Date(), index="Date")
